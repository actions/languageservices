...
export function evaluateStrategy(
  ...
  const templateContext = new TemplateContext(new TemplateValidationErrors(), getWorkflowSchema(), trace);
  ...
  const value = templateEvaluator.evaluateTemplate(templateContext, STRATEGY, token, 0, undefined);
