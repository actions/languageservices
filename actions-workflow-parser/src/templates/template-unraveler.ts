...
import {
  evaluateMappingToken,
  evaluateStringToken,
  evaluateTemplateToken,
...
export class TemplateUnraveler {
  ...
  private moveFirst(value: TemplateToken, removeBytes: number): void {
    switch (value.templateTokenType) {
      ...
      default:
        throw new Error(
          `Unexpected type '${value.typeName()}' when initializing object reader state`
        )
    }
  ...
  private moveNext(skipNestedEvents?: boolean): void {
    ...
    // Parent is a sequence
      ...
      this._current.parent?.value.templateTokenType === TokenType.Sequence
    ) {
  ...
  private unravel(expand: boolean): void {
    ...
    for (;;) {
      ...
      // Basic expression
        ...
        this._current.value.templateTokenType === TokenType.BasicExpression
      ) {
      ...
      // Sequence
        ...
        if (
          sequenceState.isEnd &&
          this._current.parent?.value.templateTokenType ===
            TokenType.BasicExpression &&
          this._current.parent.parent?.value.templateTokenType ===
            TokenType.Sequence
        ) {
          this._current = this._current.parent // Skip to the expression end
        }
      ...
      // Insert expression
        ...
        this._current.value.templateTokenType === TokenType.InsertExpression
      ) {
  ...
  private startMappingInsertion() {
    ...
    if (nestedValue.templateTokenType === TokenType.Mapping) {
      ...
      try {
        nestedMapping = evaluateMappingToken(
          basicExpression,
...
abstract class ReaderState {
  ...
  public static createState(
    ...
    switch (value.templateTokenType) {
      ...
      default:
        throw new Error(
          `Unexpected type '${value.typeName()}' when constructing reader state`
        )
    }
  }
}
